...
def test_embed_text_returns_1536_dims():
    with patch("src.rag.embedder.openai.OpenAI", return_value=_mock_openai(_FAKE_VEC)):
        vec = embed_text("software engineer")
    assert len(vec) == 1536


def test_embed_text_passes_correct_model_and_input():
    mock_client = _mock_openai(_FAKE_VEC)
    with patch("src.rag.embedder.openai.OpenAI", return_value=mock_client):
        embed_text("some bullet text")
    mock_client.embeddings.create.assert_called_once_with(
        model="text-embedding-3-small", input="some bullet text"
    )


def test_embed_text_returns_embedding_from_response():
    expected = [float(i) for i in range(1536)]
    with patch("src.rag.embedder.openai.OpenAI", return_value=_mock_openai(expected)):
